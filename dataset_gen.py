...
def loadpc(file_list):
    for file in file_list:
        ...
        for r in ratios:
            ...
            np.savetxt(
                f"customData/withNormals/{r}/{file}_{r}.xyz", pc_with_normals_fps
            )

            if not os.path.exists(f"customData/onlyNormals/{r}"):
                os.makedirs(f"customData/onlyNormals/{r}")
            np.savetxt(
                f"customData/onlyNormals/{r}/{file}_{r}.normals",
            ...
            np.savetxt(
                f"customData/withoutNormals/{r}/{file}_{r}.xyz",
                pc_with_normals_fps[:, :3],
            )
...
if __name__ == "__main__":
    ...
    # for file in file_list:
    #     path = f"customData_GT/withNormals/4096/{file}_4096.xyz"
    ...
    xx = np.loadtxt("customData_GT/withNormals/8192/Liberty100k_8192.xyz")
    res = farthest_point_sample(xx,5120)
    visualisation.visPointCloud(res[:,:3])
